...
export function CommissionSummaryCards({ summary, isLoading }: SummaryCardsProps) {
                  ...
                  <div className="mt-6">
                    <div className="flex items-center justify-between text-sm mb-2">
                      ...
                      <span className="text-white font-medium">
                        {formatCurrency(summary.totalSubtotalMxn)}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-sm mb-2">
                  ...
                  <div className="flex-1 lg:flex-none bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xl font-bold">{formatCurrency(summary.totalNetMxn)}</p>
                      </div>
                    </div>
                  </div>
                  <div className="flex-1 lg:flex-none bg-yellow-400/20 backdrop-blur-sm rounded-xl p-4 border border-yellow-400/30">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xl font-bold text-yellow-100">{formatCurrency(summary.totalRetentions)}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <CardContent className="p-0">
            <div className="p-5">
              ...
              <p className="text-3xl font-bold text-gray-900 tracking-tight">
                {formatCurrency(summary.mlmCommissionsMxn)}
              </p>
            </div>
            {/* Bottom accent */}
        ...
        <Card className="group hover:shadow-lg hover:shadow-yellow-500/10 transition-all duration-300 border-0 shadow-md overflow-hidden">
          <CardContent className="p-0">
            <div className="p-5">
              ...
              <p className="text-3xl font-bold text-gray-900 tracking-tight">
                {formatCurrency(summary.cedeaBonusesMxn)}
              </p>
        ...
        <Card className="group hover:shadow-lg hover:shadow-purple-500/10 transition-all duration-300 border-0 shadow-md overflow-hidden">
          <CardContent className="p-0">
            <div className="p-5">
              ...
              <p className="text-3xl font-bold text-gray-900 tracking-tight">
                {formatCurrency(summary.autoBonusesMxn)}
              </p>
              <div className="mt-3 pt-3 border-t border-gray-100">
                <p className="text-xs text-gray-500 flex items-center gap-1">
        ...
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-green-50 rounded-xl p-4 border border-green-100">
              ...
              <p className="font-bold text-green-600 text-lg">{formatCurrency(summary.totalSubtotalMxn)}</p>
            </div>
            <div className="bg-red-50 rounded-xl p-4 border border-red-100">
              ...
              <p className="font-bold text-red-500 text-lg">-{formatCurrency(summary.totalRetentions)}</p>
            </div>
            <div className="bg-orange-50 rounded-xl p-4 border border-orange-100">
              ...
              <p className="font-bold text-orange-600 text-lg">{formatCurrency(summary.adjustmentsMxn)}</p>
            </div>
            <div className="bg-gradient-to-br from-[#003B7A]/5 to-[#003B7A]/10 rounded-xl p-4 border border-[#003B7A]/20">
              ...
              <p className="font-bold text-[#003B7A] text-lg">{formatCurrency(summary.totalNetMxn)}</p>
            </div>
          </div>
        </CardContent>
      </Card>
